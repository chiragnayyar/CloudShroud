...
If you choose to go with a policy-based VPN, the remote partner will need to bring up individual IPSEC Security Association (SA) for each source/destination network pair defined. For example if you have defined 192.168.0.0/16,172.17.0.0/16 as the LANs behind the remote peer and your VPC is 10.0.0.0/16, then your partner will need to initiate traffic from a device in each of their two LANs to any EC2 in your VPC to bring up the SAs.
